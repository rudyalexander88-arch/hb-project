...
function aplicarPermisosDashboard() {
    ...
    const configuracionModulos = [
        ...
        { idMenu: "menuInventariosMensuales", modulo: "INVENTARIOS_MENSUALES" },
    ...
    configuracionModulos.forEach(item => {
        ...
        const permitido = item.idMenu === "menuCentroConocimiento"
            ...
            typeof window.ReportesKPIs.puedeVer === "function"
                ...
                : item.idMenu === "menuInventariosMensuales" &&
                  window.InventariosMensuales &&
                  typeof window.InventariosMensuales.puedeVer === "function"
                ? window.InventariosMensuales.puedeVer()
...
function inicializarMenu() {
    ...
    const menuInventariosMensuales = document.getElementById("menuInventariosMensuales");
    if (menuInventariosMensuales) {
        menuInventariosMensuales.addEventListener("click", async () => {
            if (!window.InventariosMensuales || typeof window.InventariosMensuales.cargar !== "function") {
                Sistema.error("El módulo de Inventarios mensuales no está disponible.");
                return;
            }
            if (!window.InventariosMensuales.puedeVer()) {
                Sistema.advertencia("No tiene acceso a Inventarios mensuales.", 4200);
                return;
            }
            activarMenu("menuInventariosMensuales");
            await window.InventariosMensuales.cargar();
        });
    }
