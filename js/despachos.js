        ...
        <div class="resumen-carga-bloque">
            ...
            <div class="resumen-inspeccion-conduce">
                ...
                <strong id="numeroInspeccionConduce">
                    Se validará al continuar
                </strong>

                <small id="estadoInspeccionConduce">
                    La inspección BASC se comprobará antes del paso 4
                </small>
...
async modalAgregarTarima() {
            ...
            <div class="grid-form">
                ...
                <div class="campo campo-full" id="campoCamaraTarima" hidden>
                    ...
                    <small id="detalleCamaraTarima">
                        La salida se registrará cuando el conduce pase a Despachado.
                    </small>
                </div>
				${
    Number(Conduce.encabezado.cantidadDestinos) === 2
    ...
    let disponibilidadCamara = {accionSinExistencia: "PERMITIR", camaras: []};
    ...
    document.getElementById("btnGuardarTarima").onclick = async () => {
        ...
        const tarimasActuales = Conduce.detalle.filter(
            linea => linea.tipo === "Tarima"
        ).length;

        if ((tarimasActuales + cantidadTarimas) > 18) {
		...
		const origenCamara = camaraSeleccionada
            ...
            : {
                idCamara: "",
                codigoCamara: "",
                observacionCamara: "Despacho registrado sin cámara de origen disponible en el control de ocupación."
            };

        const botonAgregar = document.getElementById("btnGuardarTarima");

        if (botonAgregar.dataset.procesando === "true") {
            return;
        }
        ...
        const detalleAnterior = Conduce.detalle.slice();
        const contadorAnterior = Number(Conduce.contadorLineas || 0);

        try {
            for (let i = 0; i < cantidadTarimas; i++) {
                await Despachos.agregarLinea(
                    material,
                    fechaProduccion,
                    destino,
                    "Tarima",
            ...
            if (!guardado) {
                Conduce.detalle = detalleAnterior;
                Conduce.contadorLineas = contadorAnterior;
                Despachos.normalizarDetalleCarga();
                Despachos.refrescarCarga();
                return;
            }
        } finally {
            botonAgregar.dataset.procesando = "false";
...
return true;
    ...
    // La salida física ya ocurrió. Guardamos el documento disponible y
    // solicitamos el correo ahora, sin esperar el cierre administrativo SAP.
    await Despachos
        .guardarYEnviarDocumentacionDespachada();
...
async guardarYEnviarDocumentacionDespachada() {
    ...
    try {
        ...
        if (!htmlConduce) {
            throw new Error(
                "No fue posible construir el documento del conduce."
            );
        }

        const respuestaPDF =
            await Despachos.consultarBackendConCargador({
                action: "guardarPDFConduce",
                idConduce: Conduce.encabezado.idConduce,
                noConduce: Conduce.encabezado.noConduce,
                html: htmlConduce
            }, "Guardando documentación", "El conduce ya fue despachado. Estamos preparando su correo.");

        if (!respuestaPDF || !respuestaPDF.ok) {
            Despachos.notificar(
                respuestaPDF?.mensaje ||
                "El material salió y la ocupación fue liberada, pero no se pudo guardar el PDF.",
                "advertencia"
            );
            return false;
        }
        ...
        const respuestaCorreo =
            await Despachos.consultarBackendConCargador({
                action: "enviarDocumentacionDespacho",
                idConduce: Conduce.encabezado.idConduce,
                noConduce: Conduce.encabezado.noConduce,
                usuario: sesion.usuario || sesion.nombre || ""
            }, "Enviando documentación", "El despacho ya salió del almacén. Estamos enviando el conduce por correo.");

        if (!respuestaCorreo || !respuestaCorreo.ok) {
            Despachos.notificar(
                respuestaCorreo?.mensaje ||
                "La ocupación fue liberada, pero el correo quedó pendiente.",
                "advertencia"
            );
            return false;
        }

        if (!respuestaCorreo.yaEnviado) {
            Despachos.notificar(
                "Despacho registrado, espacios liberados y documentación enviada.",
                "exito"
            );
        }
        ...
        console.error(
            "Error preparando la documentación al despachar:",
            error
        );

        Despachos.notificar(
            "El despacho y la liberación de espacios se registraron, pero la documentación quedó pendiente de envío.",
            "advertencia"
        );
