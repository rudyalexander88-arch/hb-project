...
window.GestionEPP={
 ...
 renderMovimiento(tipo){const ajuste=tipo==="AJUSTE",c=document.getElementById("eppContenido"),inv=this.datos.inventario||[];
  ...
  const motivos=this.datos.catalogos?.motivosAjuste||[];
  // TEMPORAL: mover estos motivos a Catalogos en la próxima implementación necesaria del backend.
  const motivosEntrada=["Compra de reposición","Compra por nuevo ingreso","Reposición por consumo","Recepción de proveedor","Transferencia interna","Donación"];
  ...
  <label class="ancho">Motivo${!ajuste?`<select id="eppMovimientoMotivo" required><option value="">Seleccione un motivo...</option>${motivosEntrada.map(m=>`<option value="${this.esc(m)}">${this.esc(m)}</option>`).join("")}</select>`:
 ...
 async guardarMovimiento(tipo,b){const ajuste=tipo==="AJUSTE",id=document.getElementById("eppMovimientoEquipo")?.value,q=Number(document.getElementById("eppMovimientoCantidad")?.value||0),
  ...
  const motivoRegistrado=ajuste?motivo:`Factura: ${factura}, ${motivo}`;
  await this.ejecutar(b,ajuste?"Guardando ajuste":"Registrando entrada","Actualizando las existencias del equipo.",{action:ajuste?"ajustarInventarioEPP":"registrarEntradaInventarioEPP",idEPP:id,cantidad:q,motivo:motivoRegistrado,valorUnitario:valor},
